...
#define LINK	    (5*NODES-10)		// 总共的有向边的个数number of directed links
...
void Prodgraph(void) 
{
	...
	for (i=3; i<NODES; i++) 
	{
		...
		{
		while (f<3) // meaning that a new node (i) is attached to two(f<2) old nodes
		{
			n=(int)randi(i); // an old node is choosen [e.g. randi(2) is either 0 or 1]; note that rand(i) always selects a node whose attachment is already done ate least once
			if (n!=r && n!=r2) 
			{
...
void avg_degree(){
	...
	cout<<"avg_degree = "<<sum/NODES<<endl;
}
void main()
{
